...
output_file = os.path.join(base_dir, "main_output.json")
...
# ==========================================
# 2. PHASE 1: DOWNLOADER & RAW EXTRACTOR
# ==========================================
def download_pages(page_number):
    url = "https://www.nykaa.com/brands/foxtale/c/27361?page_no={page_number}&sort=popularity"
...
# ==========================================
# 3. PHASE 2: JSON PROCESSORS & UTILITIES
# ==========================================
...
def extract_product_name(product):
    return product.get('name') if isinstance(product, dict) else None


def extract_brand_name(product):
    return product.get('brandName') if isinstance(product, dict) else None


def extract_product_url(url_item):
    return url_item.get('url') if isinstance(url_item, dict) else None

def page_count(json_data):
    product_count = json_data.get('dataLayer').get('totalFound')
    per_page = json_data.get('remoteConfig').get('configs').get('dnPaginationConfig').get('pageLimit')

    page_no = math.ceil(product_count / per_page)
    return page_no

def process_and_extract_all(data):
    if not data:
        return []

    try:
        products_list = data['categoryListing']['listingData']['products']
    except (KeyError, TypeError):
        products_list = []

    try:
        url_elements_list = data['jsonLdData'][1]['itemListElement']
    except (KeyError, IndexError, TypeError):
        url_elements_list = []

    extracted_results = []
    for product, url_item in zip(products_list, url_elements_list):
        record = {
            "product_name": extract_product_name(product),
            "brand_name": extract_brand_name(product),
            "product_url": extract_product_url(url_item)
        }
...
# ==========================================
# 4. PHASE 3: DATABASE BACKEND
# ==========================================
def save_sql(product_list):
    if not product_list:
        print("No products found to save to MySQL.")
        return
    ...
    try:
        ...
        if connection.is_connected():
            ...
            # Fixed SQL Statement: Syntax adjusted from "ADD UNIQUE VARCHAR" to proper SQL standards.
            # Using 768 length because it is the standard maximum size limit for MySQL unique index keys.
            create_table_query = """
            CREATE TABLE IF NOT EXISTS products (
                id INT AUTO_INCREMENT PRIMARY KEY,
                product_name VARCHAR(500) NOT NULL,
                brand_name VARCHAR(100),
                product_url VARCHAR(768) UNIQUE
            );
            """
            cursor.execute(create_table_query)
            print("Table 'products' verified/created successfully.")

            insert_query = """
            INSERT IGNORE INTO products (product_name, brand_name, product_url) 
            VALUES (%(product_name)s, %(brand_name)s, %(product_url)s);
            """
            
            cursor.executemany(insert_query, product_list)
            connection.commit()
            print(f"Successfully inserted {cursor.rowcount} products into MySQL!")

    except Error as e:
        print(f"Error while operating on MySQL: {e}")
...
# ==========================================
# 5. UNIFIED PIPELINE EXECUTION ENTRYPOINT
# ==========================================
if __name__ == "__main__":
    # --- STEP 1: DOWNLOAD RAW HTML & INITIALIZE PATHS ---
    initial_page = 1
    download_pages(initial_page)
    
    # Define paths for the first page to extract total page count safely
    first_html_file = os.path.join(html_dir, f"{initial_page}_source_code.html")
    first_json_file = os.path.join(json_dir, f"{initial_page}_preloaded_state.json")
    
    # Extract the JSON for page 1 so we can read the page count
    extract_preloaded_state(first_html_file, first_json_file)
    
    # Read the data back to calculate total pages dynamically
    initial_json_data = read_json_file(first_json_file)
    total_page = page_count(initial_json_data)
    
    print(f"Total pages detected from data: {total_page}")

    # --- STEP 2: DOWNLOAD & EXTRACT JAVASCRIPT STATE ARRAYS INTO RAW JSON FILES ---
    print("\n" + "="*40)
    ...
    for page_no in range(1, total_page + 1):
        ...
        # If it's a new page we haven't downloaded yet during the loop, download it
        if not os.path.exists(html_file):
            download_pages(page_no)
    ...
    # --- STEP 3: REPARSE INDIVIDUAL PAGES AND COMBINE MAPS ---
    print("\n" + "="*40)
    print("=== Phase 3: Processing Data Streams & Storage ===")
    print("="*40 + "\n")

    all_extracted_records = []
    json_files = sorted([f for f in os.listdir(json_dir) if f.endswith('.json')])
    print(f"Found {len(json_files)} JSON file(s) in folder to process.")
    ...
    for file_name in json_files:
        ...
        page_output = process_and_extract_all(raw_data)
        print(f" -> Found {len(page_output)} products.")
        
        all_extracted_records.extend(page_output)
        
    print(f"\nTotal collected records across all files: {len(all_extracted_records)}")
    
    # --- STEP 4: WRITE UNIFIED FLAT BACKUP FILE ---
    try:
        with open(output_file, 'w', encoding='utf-8') as out_file:
            json.dump(all_extracted_records, out_file, indent=2, ensure_ascii=False)
        print(f"Successfully saved all combined data to '{output_file}'")
    except Exception as e:
        print(f"Error saving output backup file: {e}")
        
    # --- STEP 5: SEED MYSQL DATABASE ---
    save_sql(all_extracted_records)